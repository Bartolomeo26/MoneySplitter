...
async def add_participant(request: Request, session_id: str, participant_name: str = Form(...)):
    session = storage.get_session(session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Sesja nie została znaleziona")
    
    if session.read_only:
        raise HTTPException(status_code=403, detail="Sesja jest tylko do odczytu")
    ...
    participants_html = templates.get_template("partials/participants_list.html").render(
        request=request,
        session=session,
        settlement=settlement
    )
    
    expense_form_html = templates.get_template("partials/expense_form.html").render(
        request=request,
        session=session,
        settlement=settlement
    )
    
    expense_form_oob = expense_form_html.replace('<div id="expense-form-section">', '<div id="expense-form-section" hx-swap-oob="true">', 1)
...
async def add_expense(
    ...
    if payer_id not in participant_ids:
        raise HTTPException(status_code=400, detail="Płatnik musi być uczestnikiem sesji")
    
    for bid in beneficiary_ids:
        if bid not in participant_ids:
            raise HTTPException(status_code=400, detail="Wszyscy beneficjenci muszą być uczestnikami sesji")
...
async def export_csv(session_id: str):
    ...
    writer.writerow([f"Data utworzenia: {session.created_at.strftime('%Y-%m-%d %H:%M')}"])
    writer.writerow([])
    ...
    for expense in session.expenses:
        beneficiaries_names = ", ".join([participant_map[bid] for bid in expense.beneficiary_ids])
        writer.writerow([
            expense.date.strftime('%Y-%m-%d'),
            expense.title,
            format_currency(expense.amount_minor),
            participant_map[expense.payer_id],
            beneficiaries_names
        ])
    writer.writerow([])
    ...
    for balance in settlement.balances:
        writer.writerow([balance.participant_name, format_currency(balance.balance_minor)])
    writer.writerow([])
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=rozliczenie_{session.name.replace(' ', '_')}.csv"}
    )
...
async def export_pdf(session_id: str):
    ...
    elements.append(Paragraph(f"Sesja: {session.name}", styles['Normal']))
    elements.append(Paragraph(f"Data utworzenia: {session.created_at.strftime('%Y-%m-%d %H:%M')}", styles['Normal']))
    ...
    expense_table = Table(expense_data, colWidths=[2.5*cm, 4*cm, 2.5*cm, 3*cm, 3*cm])
    ...
    for balance in settlement.balances:
        balance_data.append([balance.participant_name, format_currency(balance.balance_minor)])
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=rozliczenie_{session.name.replace(' ', '_')}.pdf"}
    )
